...
class UserAuthFilterTest {
    ...
    @InjectMocks
    UserAuthFilter filter;

    @BeforeEach
    void setUp() {
        MockitoAnnotations.openMocks(this);
        filter = new UserAuthFilter();
    }
    ...
    void testDoFilter_missingAuthorizationHeader() throws Exception {
        ...
        verify(response).sendError(eq(403), contains("Authorization token must be provide for user"));
    ...
    void testDoFilter_invalidToken() throws Exception {
        ...
        verify(response).sendError(eq(403), contains("Invalid / Expired user token"));
        verify(chain, never()).doFilter(any(), any());
    }
    ...
    void testDoFilter_authorizationHeaderWithoutBearerToken() throws Exception {
        ...
        verify(response).sendError(eq(403), contains("Authorization token must be Bearer in user token"));
        verify(chain, never()).doFilter(any(), any());
    }
}