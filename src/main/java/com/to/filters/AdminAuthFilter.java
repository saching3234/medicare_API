	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		if (authHeader != null) {
			...
			if (authHeaderArr.length > 1 && authHeaderArr[1] != null) {
				...
				try {
					// checking the token and setting the admin id in the http request
					Claims claims = Jwts.parser().setSigningKey(Constants.API_SECRET_KEY_STRING).parseClaimsJws(token)
							.getBody();
					//setting the adminId to httpServletRequest for accessing the admin id.
					httpServletRequest.setAttribute("adminId", claims.get("adminId").toString());
					System.out.println("Admin filter : admin Id : " + claims.get("adminId").toString());
				...
				httpServletResponse.sendError(HttpStatus.FORBIDDEN.value(),
						"Authorization token must be Bearer in admin token");
				return;
			}

		} else {
			httpServletResponse.sendError(HttpStatus.FORBIDDEN.value(),
					"Authorization token must be provide for admin");
			return;
		}
